...
import { TadiranAccessory } from './accessory.js';
import { PLUGIN_NAME, PLATFORM_NAME, DEFAULT_POLL_INTERVAL } from './constants.js';
import { normalizePhone, maskPhone } from './phone.js';
import { authFilePath, consumeFactoryReset, isFactoryResetPending } from './auth-storage.js';

export function normalizePollInterval(value) {
  if (!['number', 'string'].includes(typeof value) || String(value).trim() === '') return DEFAULT_POLL_INTERVAL;
  const parsed = Number(value);
  if (!Number.isFinite(parsed)) return DEFAULT_POLL_INTERVAL;
  return Math.min(300, Math.max(15, Math.round(parsed)));
}

export class MyTadiranPlatform {
  constructor(log, config, api) {
    ...
    this.retryDelay = 30;
    this.debugLogs = Boolean(this.config.debugLogs);

    this.pollInterval = normalizePollInterval(this.config.pollInterval);
    ...
    this.persistPath = this.api.user.persistPath();
    this.authFile = authFilePath(this.persistPath);

    try {
      if (consumeFactoryReset(this.persistPath)) {
        this.clearClientAuthentication();
        this.log.warn('My Tadiran factory reset completed. Enter a phone number in the plugin settings to link the account again.');
      }
    } catch (error) {
      this.log.error(`Could not complete My Tadiran reset: ${error.message}`);
      return;
    }
  ...
  debug(message) {
    if (this.debugLogs) this.log.info(`[debug] ${message}`);
  }
  ...
  writeAuthState(data) {
    try {
      if (this.factoryResetPending() || this.stopped) return;
  ...
  clearClientAuthentication() {
    ...
    this.client.idToken = null;
    this.client.refreshToken = null;
    this.client.orgId = null;
  }

  factoryResetPending() {
    return isFactoryResetPending(this.persistPath);
  }
  ...
  async authenticate() {
    ...
    if (saved.refreshToken) {
      ...
      try {
        ...
        if (!(error instanceof TadiranNotAuthorized)) throw error;
    ...
    if (saved.otpSession) {
      ...
      try {
        ...
        if (error instanceof TadiranInvalidSession) {
          // Cognito can invalidate a CUSTOM_AUTH challenge session before the
          // user submits the SMS code. Replace the stale session immediately
          // so the user only needs to enter the newly sent code and restart.
          try { fs.unlinkSync(this.authFile); } catch {}
          const session = await this.client.initiateOtp(this.phone);
          this.writeAuthState({ otpSession: session });
          this.log.warn(`The previous My Tadiran SMS login session was no longer valid. A fresh SMS code was sent to ${maskPhone(this.phone)}. Enter the new code in “One-time SMS code (OTP)” and save/restart Homebridge.`);
          return false;
        }
  ...
  async start() {
    if (this.stopped || this.starting || this.pollTimer) return;
    if (this.retryTimer) clearTimeout(this.retryTimer);
    this.retryTimer = null;
    this.starting = true;
    try {
      if (this.factoryResetPending()) return;
      if (!(await this.authenticate())) return;
      if (this.stopped || this.factoryResetPending()) return;
      this.retryDelay = 30;
      ...
      // Retry only a saved-token refresh. Never repeatedly send SMS codes or
      // replay an OTP after an ambiguous network failure.
      if (!this.stopped && isTransientAuthError(error) && this.readAuthState().refreshToken) {
        const delay = this.retryDelay;
        this.retryDelay = Math.min(300, delay * 2);
        this.log.warn(`Saved My Tadiran login kept. Retrying startup in ${delay} seconds.`);
  ...
  async refreshDevices(initial = false) {
    ...
    this.polling = true;
    try {
      if (this.factoryResetPending()) {
        this.clearClientAuthentication();
        try { fs.unlinkSync(this.authFile); } catch {}
        if (this.pollTimer) clearInterval(this.pollTimer);
        this.pollTimer = null;
        this.log.warn('My Tadiran factory reset is pending. Cloud activity is paused until Homebridge restarts.');
        return;
      }

      const devices = await this.client.getDevices();
      if (this.stopped) return;
      if (initial) this.debug(`Discovered ${devices.length} My Tadiran device(s).`);
      const seen = new Set();
      ...
      // A factory-reset request can arrive while a cloud poll is in flight.
      // Never recreate the refresh-token file after the reset UI has deleted it.
      if (this.factoryResetPending()) {
        this.clearClientAuthentication();
        try { fs.unlinkSync(this.authFile); } catch {}
        if (this.pollTimer) clearInterval(this.pollTimer);
