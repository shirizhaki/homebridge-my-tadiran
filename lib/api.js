...
export class TadiranTransientAuthError extends TadiranAuthError {}
export class TadiranInvalidOTP extends TadiranAuthError {}
export class TadiranNotAuthorized extends TadiranAuthError {}
export class TadiranInvalidSession extends TadiranNotAuthorized {}
export class TadiranAPIError extends Error {}

export function isTransientAuthError(error) {
  return error instanceof TadiranTransientAuthError
...
export class TadiranAPI {
  constructor(log, options = {}) {
    ...
    this.requestTimeoutMs = Number.isSafeInteger(configuredTimeout) && configuredTimeout > 0
      ? configuredTimeout
      : DEFAULT_REQUEST_TIMEOUT;
  ...
  async cognito(target, body) {
    ...
    if (!response.ok) {
      ...
      if (response.status >= 500 || response.status === 429 || /TooManyRequests|InternalError|ServiceUnavailable/.test(String(type))) {
        throw new TadiranTransientAuthError(`${type}: ${message}`);
      }
      if (String(type).includes('CodeMismatch') || String(type).includes('ExpiredCode')) {
        throw new TadiranInvalidOTP(`${type}: ${message}`);
      }
      if (target === 'RespondToAuthChallenge' && String(type).includes('NotAuthorizedException') && /invalid session/i.test(String(message))) {
        throw new TadiranInvalidSession(`${type}: ${message}`);
      }
      if (String(type).includes('NotAuthorizedException')) {
        throw new TadiranNotAuthorized(`${type}: ${message}`);
      }
  ...
  async verifyOtp(phone, session, otp) {
    ...
    while (data.ChallengeName && !data.AuthenticationResult && challengeCount < MAX_AUTH_CHALLENGES) {
    ...
    if (!data.AuthenticationResult) {
      const detail = data.ChallengeName
        ? `Cognito challenge limit (${MAX_AUTH_CHALLENGES}) reached`
        : 'No AuthenticationResult in Cognito response';
      throw new TadiranAuthError(detail);
    }
